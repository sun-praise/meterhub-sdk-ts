...
import { APIPromise } from 'meterhub/core/api-promise';

import util from 'node:util';
import Meterhub from 'meterhub';
import { APIUserAbortError } from 'meterhub';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['METERHUB_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: Meterhub) => {
    ...
    test('debug logs when log level is debug', async () => {
      ...
      const client = new Meterhub({ logger: logger, logLevel: 'debug', apiKey: 'My API Key' });
    ...
    test('default logLevel is warn', async () => {
      const client = new Meterhub({ apiKey: 'My API Key' });
    ...
    test('debug logs are skipped when log level is info', async () => {
      ...
      const client = new Meterhub({ logger: logger, logLevel: 'info', apiKey: 'My API Key' });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['METERHUB_LOG'] = 'debug';
      const client = new Meterhub({ logger: logger, apiKey: 'My API Key' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['METERHUB_LOG'] = 'not a log level';
      const client = new Meterhub({ logger: logger, apiKey: 'My API Key' });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'METERHUB_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
    ...
    test('client log level overrides env var', async () => {
      ...
      process.env['METERHUB_LOG'] = 'debug';
      const client = new Meterhub({ logger: logger, logLevel: 'off', apiKey: 'My API Key' });
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new Meterhub({ baseURL: 'http://localhost:5000/custom/path/', apiKey: 'My API Key' });
    ...
    afterEach(() => {
      process.env['METERHUB_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new Meterhub({ baseURL: 'https://example.com', apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com');
    });

    test('env variable', () => {
      process.env['METERHUB_BASE_URL'] = 'https://example.com/from_env';
      const client = new Meterhub({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com/from_env');
    });

    test('empty env variable', () => {
      process.env['METERHUB_BASE_URL'] = ''; // empty
      const client = new Meterhub({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://api.example.com');
    });

    test('blank env variable', () => {
      process.env['METERHUB_BASE_URL'] = '  '; // blank
      const client = new Meterhub({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://api.example.com');
    });
    ...
    test('in request options overridden by client options', () => {
      const client = new Meterhub({ apiKey: 'My API Key', baseURL: 'http://localhost:5000/client' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
    ...
    test('in request options overridden by env variable', () => {
      process.env['METERHUB_BASE_URL'] = 'http://localhost:5000/env';
      const client = new Meterhub({ apiKey: 'My API Key' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
  ...
  test('maxRetries option is correctly set', () => {
    const client = new Meterhub({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new Meterhub({ apiKey: 'My API Key' });
    expect(client2.maxRetries).toEqual(2);
  });

  describe('withOptions', () => {
    test('creates a new client with overridden options', () => {
      const client = new Meterhub({ baseURL: 'http://localhost:5000/', maxRetries: 3, apiKey: 'My API Key' });
    ...
    test('respects runtime property changes when creating new client', () => {
      const client = new Meterhub({ baseURL: 'http://localhost:5000/', timeout: 1000, apiKey: 'My API Key' });
  ...
  test('with environment variable arguments', () => {
    ...
    process.env['METERHUB_API_KEY'] = 'My API Key';
    const client = new Meterhub();
  ...
  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['METERHUB_API_KEY'] = 'another My API Key';
    const client = new Meterhub({ apiKey: 'My API Key' });
    expect(client.apiKey).toBe('My API Key');
  });
});
...
describe('retries', () => {
  test('retry on timeout', async () => {
    ...
    const client = new Meterhub({ apiKey: 'My API Key', timeout: 10, fetch: testFetch });
  ...
  test('retry count header', async () => {
    ...
    const client = new Meterhub({ apiKey: 'My API Key', fetch: testFetch, maxRetries: 4 });
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new Meterhub({ apiKey: 'My API Key', fetch: testFetch });

...
await client.api.v1.meter.detect({ image: 'REPLACE_ME' }, {
  maxRetries: 5,
});
```
...
const response = await client.api.v1.meter.detect({ image: 'REPLACE_ME' }).asResponse();
console.log(response.headers.get('X-My-Header'));
console.log(response.statusText); // access the underlying Response object

const { data: response, response: raw } = await client.api.v1.meter
  .detect({ image: 'REPLACE_ME' })
  .withResponse();
...
1. Via the `METERHUB_LOG` environment variable
...
const client = new Meterhub({
  logger: logger.child({ name: 'Meterhub' }),
...
const client = new Meterhub({ fetch });
```
...
```ts
import Meterhub from 'npm:meterhub';
